...
public class Map4<K, V> extends MapSecondary<K, V> {
    ...
    private static int mod(int a, int b) {
        ...
        // Loop while a is less than 0 or |a| is greater than modulo for "clock arithmetic"
        while (aCopy > 0 || Math.abs(aCopy) > b) {
            if (aCopy > 0) {
                aCopy -= b;
            } else {
                aCopy += b;
    ...
    public final void add(K key, V value) {
        ...
        // Find bucket and add to map entry
    ...
    public final Pair<K, V> remove(K key) {
        ...
        // Find bucket and remove from map entry
        int bucket = mod(key.hashCode(), this.hashTable.length());
    ...
    public final Pair<K, V> removeAny() {
        ...
        boolean firstBucket = false;
        Map<K, V> mapAlias = new Map2<>();

        // Iterate to find first bucket with any value
        for (Map<K, V> x : this.hashTable) {
            // Find first non-empty bucket and alias it
            if (!firstBucket && x.size() > 0) {
                firstBucket = true;
                mapAlias = x;
            }
        }

        // Remove and return from alias
        return mapAlias.removeAny();
    }
    ...
    public final boolean hasKey(K key) {
        ...
        // Find bucket and determine if key exists in entry
        int bucket = mod(key.hashCode(), this.hashTable.length());
    ...
    public final int size() {
        ...
        // Iterate through hashTable and add size of each entry
        for (Map<K, V> x : this.hashTable) {
            mapSize += x.size();
        }
        return mapSize;
