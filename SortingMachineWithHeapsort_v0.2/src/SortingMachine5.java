...
public class SortingMachine5<T> extends SortingMachineSecondary<T> {
    ...
    private static <T> void siftDown(Array<T> array, int top, int last,
        ...
        if (top < last) {
            ...
            if (!isHeap(array, top, last, order)) {
                // Determine which tree to examine and swap nodes if applicable
                if ((order.compare(array.entry(left), array.entry(right)) < 0
                        && left <= last) || right > last) {
                    if (order.compare(array.entry(top),
                            array.entry(left)) > 0) {
                        T hold = array.entry(top);
                        array.replaceEntry(top, array.entry(left));
                        array.replaceEntry(left, hold);
                    ...
                    if ((order.compare(array.entry(top), array.entry(right)) > 0
                        ...
                        T hold = array.entry(top);
                        array.replaceEntry(top, array.entry(right));
                        array.replaceEntry(right, hold);
    ...
    private static <T> void heapify(Array<T> array, int top,
        ...
        if (!isHeap(array, top, last, order)) {
            // Look if right and left subtrees are heaps, if not, heapify
            if (!isHeap(array, left, last, order)) {
                heapify(array, left, order);
            }
            if (!isHeap(array, right, last, order)) {
                heapify(array, right, order);
            }

            // Once subtrees are heapified, sift down top node
    ...
    private static <T> Array<T> buildHeap(Queue<T> q, Comparator<T> order) {
        ...
        // Transfer queued items into array and heapify array
        Array<T> toHeap = new Array1L<>(q.length());
        for (int i = 0; i < q.length(); i++) {
            toHeap.setEntry(i, q.dequeue());
        }
        heapify(toHeap, 0, order);
    ...
    public final T removeFirst() {
        ...
        int newHeapSize = this.heapSize - 1;
        Array<T> newHeap = new Array1L<>(newHeapSize);
        T removed = this.heap.entry(0);

        for (int i = 1; i < this.heapSize; i++) {
            newHeap.setEntry(i - 1, this.heap.entry(i));
        }
    ...
    public final int size() {
        // Determine which to return as size based on insertion mode
        int sortingMachineSize = 0;
        if (this.isInInsertionMode()) {
            sortingMachineSize = this.entries.length();
        } else {
            sortingMachineSize = this.heapSize;
        }

        assert this.conventionHolds();
        // Fix this line to return the result after checking the convention.
        return sortingMachineSize;
    }
